...
void initGraph(nod* N, const RRBoard* board, nod** Tab){
    //
    //nb_elem_tab3 ++;
    //Tab3 = (nod****)malloc(sizeof(nod*)*100*100*5);

    for(int i = 0 ;  i < 50000; ++i)
                    Tab3[i] = NULL;

    Tab3[N->data->line + N->data->column*LINE + N->data->status*LINE*COL] = N;
    puts("caca");
    ...
    Tab3[mort->data->line + mort->data->column*LINE + mort->data->status*LINE*COL] = mort;
...
newHorizon(int i, nod* N, RRRobotMove move, nod** Tab, const RRBoard* board){
    ...
    if(temp->status == RR_ROBOT_DEAD) {
        ...
        if ((replace = inTab3(temp)) != NULL) {
            ...
            addTab(N->neib[i], Tab);
            Tab3[N->neib[i]->data->line + N->neib[i]->data->column * LINE + N->neib[i]->data->status*LINE*COL] = N->neib[i];
        }
    }
...
inTab3(RRRobot* bot){
    ///////////////////////////////////
    return Tab3[bot->line + bot->column * LINE + bot->status* LINE*COL];
}
...
findNode(int line, int column, RRRobotStatus status){
    /////////////////
    return Tab3[line + column *LINE + status*LINE*COL];
}
