...
void addToRet(RRRobotMove* ret, int retsize, RRRobotMove* sequence){
    for(int i = 0 ; i < 5; ++i)
        ret[i + retsize] = sequence[i];
    puts("caca");
}
nod* whosNext(nod* N, nod* goal, RRRobotMove** permutations, RRRobotMove* sequence, RRBoard* board){
        ...
        for (int j = 0; j < 5; ++j) {
            ...
            if(temp->line == goal->data->line
               ...
               && temp->status == goal->data->status){
                ...
                nod* best = inTab3(temp);
                //free(temp);
                for(int k = 0 ; k < 5; ++k)
                    move_print(sequence[k]);
                addToRet(ret, retsize, sequence);
                retsize +=5;
                return best;
            }
            rr_board_play(board, temp, permutations[i][j]);
        }
        if(temp->status != RR_ROBOT_DEAD && dist > (newdist = inTab3(temp)->goaldist) && temp->status != RR_ROBOT_DEAD){
            dist = newdist;
            free(bestRobot);
            //bestRobot = temp;
            copySequence(sequence, permutations[i]);
    ...
    nod* best = inTab3(bestRobot);
    for(int i = 0 ; i < 5; ++i)
        move_print(sequence[i]);
    addToRet(ret, retsize, sequence);
    retsize +=5;
    return best;
}
RRRobotMove* dontStopTillGetEnough(nod* N, nod* goal, RRBoard* board){
